# 知识银河/KnowledgeGalaxy

知识银河是一个研究“领域关系如何生成三维知识空间”的实验项目。当前模型把所有节点统一表示为 ResearchField，以层级 H、方向性知识依赖 D 和对称相关性 R 作为原始关系，再由这些关系推导依赖深度、几何亲和、三维坐标和层级密度区域。坐标是可重复计算的布局结果，不是知识事实；几何重叠也不会生成层级关系。
